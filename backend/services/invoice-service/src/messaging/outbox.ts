...
['SIGINT', 'SIGTERM'].forEach(signal => {
  process.once(signal, () => {
    shutdown().catch(error => {
      console.error('invoice-service messaging shutdown failed', error);
    });
  });
});

export const enqueueInvoiceEvent = async (eventType: string, payload: Record<string, unknown>) => {
...
export const getInvoiceEvents = () => inMemoryPublisher?.sentMessages ?? [];
