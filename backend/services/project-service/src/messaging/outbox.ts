...
['SIGINT', 'SIGTERM'].forEach(signal => {
  process.once(signal, () => {
    shutdown().catch(error => {
      console.error('project-service messaging shutdown failed', error);
    });
  });
});

export const enqueueProjectEvent = async (eventType: string, payload: Record<string, unknown>) => {
...
export const getPublishedEvents = () => inMemoryPublisher?.sentMessages ?? [];
