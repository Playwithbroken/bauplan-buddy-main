...
const IntegrationSettingsDialog: React.FC<IntegrationSettingsDialogProps> = ({
  ...
  const [calendarWebhookUrls, setCalendarWebhookUrls] = useState<Record<string, string>>({});
  const [accountingWebhookUrls, setAccountingWebhookUrls] = useState<Record<string, string>>({});

  const refreshData = useCallback(() => {
    const calendars = calendarService.getProviders();
    const accounting = accountingService.getProviders();

    setCalendarProviders(calendars);
    setAccountingProviders(accounting);
    setCalendarWebhookUrls(
      calendars.reduce<Record<string, string>>((acc, provider) => {
        acc[provider.id] = provider.auth?.webhookUrl ?? '';
        return acc;
      }, {})
    );
    setAccountingWebhookUrls(
      accounting.reduce<Record<string, string>>((acc, provider) => {
        acc[provider.id] = provider.webhookUrl ?? '';
        return acc;
      }, {})
    );
  }, []);
  ...
  const handleCalendarRefreshToken = (providerId: string) => {
    const refreshed = calendarService.refreshAccessToken(providerId);
    toast({
      title: refreshed ? 'Zugriff aktualisiert' : 'Aktualisierung fehlgeschlagen',
      description: refreshed
        ? 'Ein neues Zugriffstoken wurde erstellt.'
        : 'Das Zugriffstoken konnte nicht erneuert werden.',
      variant: refreshed ? 'default' : 'destructive'
    });
    refreshData();
  };

  const handleCalendarWebhookToggle = (providerId: string, active: boolean) => {
    calendarService.toggleWebhook(providerId, active);
    refreshData();
    toast({
      title: active ? 'Webhook aktiviert' : 'Webhook deaktiviert',
      description: active
        ? 'Echtzeit-Benachrichtigungen werden zugestellt.'
        : 'Webhook-Benachrichtigungen wurden deaktiviert.'
    });
  };

  const handleCalendarWebhookSave = (providerId: string) => {
    const url = calendarWebhookUrls[providerId]?.trim();
    calendarService.updateWebhook(providerId, url || undefined);
    refreshData();
    toast({
      title: 'Webhook aktualisiert',
      description: url ? `Endpoint ${url} gespeichert.` : 'Webhook-URL entfernt.'
    });
  };
  ...
  const handleAccountingRefreshToken = (providerId: string) => {
    const refreshed = accountingService.refreshAccessToken(providerId);
    toast({
      title: refreshed ? 'Zugriff aktualisiert' : 'Aktualisierung fehlgeschlagen',
      description: refreshed
        ? 'Das Zugriffstoken wurde erneuert.'
  ...
  const handleAccountingWebhookToggle = (providerId: string, active: boolean) => {
    accountingService.toggleWebhook(providerId, active);
    refreshData();
    toast({
      title: active ? 'Webhook aktiviert' : 'Webhook deaktiviert',
      description: active
        ? 'Webhook-Ereignisse werden versendet.'
        : 'Webhook-Ereignisse wurden deaktiviert.'
    });
  };

  const handleAccountingWebhookSave = (providerId: string) => {
    const url = accountingWebhookUrls[providerId]?.trim();
    accountingService.updateWebhook(providerId, url || undefined);
    refreshData();
    toast({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle>Integrationen verwalten</DialogTitle>
          <DialogDescription>
            Verbinden Sie Bauplan Buddy mit externen Systemen und steuern Sie Synchronisationseinstellungen.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>
                    {calendarStats.connected > 0
                      ? `${calendarStats.connected} verbundene Kalender`
                      : 'Keine aktiven Kalenderverbindungen'}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-2 text-sm">
                  <div className="flex items-center justify-between">
                    <span>Automatische Synchronisation</span>
                    <Badge variant="outline">{calendarStats.autoSync}</Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Verbindungen</span>
                    <Badge variant="secondary">{calendarProviders.length}</Badge>
                  </div>
                </CardContent>
              </Card>
              ...
              {calendarProviders.length === 0 ? (
                <Card className="border-dashed">
                  <CardContent className="flex flex-col items-center gap-3 py-12 text-center text-sm text-muted-foreground">
                    ...
                    <p>Es sind derzeit keine Kalender integriert. Verbinden Sie einen Anbieter, um Termine zu synchronisieren.</p>
                  </CardContent>
                </Card>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {calendarProviders.map((provider) => {
                    const tokenExpired = Boolean(provider.auth?.expiresAt && provider.auth.expiresAt < new Date());
                    const webhookUrl = calendarWebhookUrls[provider.id] ?? '';
                    return (
                      ...
                      <CardHeader className="pb-3">
                        <CardTitle className="flex items-center justify-between text-base">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant={provider.authenticated ? 'secondary' : 'outline'}>
                              {provider.authenticated ? 'Verbunden' : 'Nicht verbunden'}
                            </Badge>
                          </div>
                        </CardTitle>
                        <CardDescription>
                          Konto: {provider.accountEmail || 'Noch nicht authentifiziert'}
                        </CardDescription>
                      </CardHeader>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                          <div className="space-y-3 rounded-lg border p-3">
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                  Synchronisation im Hintergrund aktivieren oder deaktivieren.
                                </p>
                              </div>
                              <Switch
                                checked={provider.syncSettings.autoSync}
                                disabled={!provider.authenticated}
                                onCheckedChange={(checked) =>
                                  handleCalendarToggle(provider, 'syncSettings.autoSync', checked)
                                }
                              />
                            </div>
                            ...
                            <div className="space-y-2">
                              ...
                              <Select
                                value={provider.syncSettings.syncDirection}
                                onValueChange={(value) => handleCalendarSyncDirectionChange(provider, value as CalendarSyncDirection)}
                              >
                                <SelectTrigger>
                            ...
                            <div className="space-y-2">
                              ...
                              <Select
                                value={String(provider.syncSettings.syncFrequency)}
                                onValueChange={(value) => handleCalendarFrequencyChange(provider, Number(value))}
                              >
                                <SelectTrigger>
                          ...
                          <div className="space-y-3 rounded-lg border p-3">
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                  Steuert, ob Termine synchronisiert werden duerfen.
                                </p>
                              </div>
                              <Switch
                                checked={provider.enabled}
                                disabled={!provider.authenticated}
                                onCheckedChange={(checked) => handleCalendarToggle(provider, 'enabled', checked)}
                              />
                            </div>

                            <div className="rounded-md bg-muted p-3 text-sm">
                              <p className="flex items-center gap-2">
                                <ShieldCheck className="h-4 w-4" /> Letzte Synchronisation: {formatDateTime(provider.lastSync)}
                              </p>
                              <div className="mt-1 space-y-1 text-xs text-muted-foreground">
                                <p className="flex items-center gap-2">
                                  <Signal className="h-4 w-4" /> Status: {provider.syncSettings.autoSync ? 'Automatisch' : 'Manuell'}
                                </p>
                                <p>
                                  Token: {provider.auth?.expiresAt ? formatDateTime(provider.auth.expiresAt) : 'Nicht gesetzt'}
                                  {tokenExpired && (
                                    <Badge variant="destructive" className="ml-2">Abgelaufen</Badge>
                                  )}
                                </p>
                                {provider.auth?.lastError && (
                                  <p className="text-destructive">Fehler: {provider.auth.lastError}</p>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              <Button
                                ...
                                disabled={!provider.authenticated || syncingProviderId === provider.id}
                                className="flex items-center gap-2"
                              >
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleCalendarRefreshToken(provider.id)}
                                className="flex items-center gap-2"
                              >
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleCalendarDisconnect(provider.id)}
                            ...
                            <div className="space-y-2 rounded-md border p-3">
                              <div className="flex items-center justify-between">
                                <div>
                                  <p className="text-sm font-medium">Webhook aktiv</p>
                                  <p className="text-xs text-muted-foreground">
                                    Benachrichtigungen bei Kalender-Aenderungen.
                                  </p>
                                </div>
                                <Switch
                                  checked={provider.auth?.webhookActive ?? false}
                                  onCheckedChange={(checked) => handleCalendarWebhookToggle(provider.id, checked)}
                                />
                              </div>
                              <div className="space-y-1">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    onClick={() => handleCalendarWebhookSave(provider.id)}
                                    className="flex items-center gap-2"
                                  >
          ...
          <TabsContent value="accounting" className="space-y-6 pt-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="text-base">Status</CardTitle>
                  <CardDescription>
                    {accountingStats.enabled > 0
                      ? `${accountingStats.enabled} aktive Integrationen`
                      : 'Keine Buchhaltungsintegration aktiv'}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-2 text-sm">
                  <div className="flex items-center justify-between">
                    <span>Automatische Synchronisation</span>
                    <Badge variant="outline">{accountingStats.autoSync}</Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Verfuegbare Anbieter</span>
                    <Badge variant="secondary">{accountingProviders.length}</Badge>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>
                    Authentifizieren Sie einen Anbieter, um Debitoren, Kreditoren und Belege zu synchronisieren.
                  </CardDescription>
                </CardHeader>
                <CardContent className="text-sm text-muted-foreground">
                  Sie koennen mehrere Systeme parallel betreiben und die Synchronisation je Mandant steuern.
                </CardContent>
              </Card>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {accountingProviders.map((provider) => {
                const tokenExpired = Boolean(provider.credentials.tokenExpiry && provider.credentials.tokenExpiry < new Date());
                const webhookUrl = accountingWebhookUrls[provider.id] ?? '';
                return (
                  <Card key={provider.id}>
                  <CardHeader className="pb-3">
                    <CardTitle className="flex items-center justify-between text-base">
                    ...
                    <CardDescription>
                      API-Endpunkt: {provider.apiEndpoint}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      <div className="space-y-3 rounded-lg border p-3">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">Steuert den Datenabgleich mit {provider.name}.</p>
                          </div>
                          <Switch
                            checked={provider.enabled}
                            onCheckedChange={(checked) => handleAccountingToggle(provider, 'enabled', checked)}
                          />
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-medium">Automatische Synchronisation</p>
                            <p className="text-xs text-muted-foreground">Periodischer Sync ueber Hintergrundprozesse.</p>
                          </div>
                          <Switch
                            checked={provider.syncSettings.autoSync}
                            disabled={!provider.authenticated}
                            onCheckedChange={(checked) => handleAccountingToggle(provider, 'syncSettings.autoSync', checked)}
                          />
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            value={provider.syncSettings.syncDirection}
                            onValueChange={(value) => handleAccountingSyncDirectionChange(provider, value as AccountingSyncDirection)}
                          >
                            <SelectTrigger>
                              <SelectValue placeholder="Modus waehlen" />
                            </SelectTrigger>
                            <SelectContent>
                              {ACCOUNTING_SYNC_OPTIONS.map((option) => (
                                <SelectItem key={option.value} value={option.value}>
                                  {option.label}
                                </SelectItem>
                              ))}
                        ...
                        <div className="space-y-2">
                          <Label className="text-xs uppercase tracking-wider text-muted-foreground">Intervall (Minuten)</Label>
                          <Select
                            value={String(provider.syncSettings.syncInterval)}
                            onValueChange={(value) => handleAccountingIntervalChange(provider, Number(value))}
                          >
                            <SelectTrigger>
                              <SelectValue placeholder="Intervall waehlen" />
                            </SelectTrigger>
                      ...
                      <div className="space-y-3 rounded-lg border p-3">
                        <div className="rounded-md bg-muted p-3 text-sm">
                          <p className="flex items-center gap-2">
                            <RefreshCcw className="h-4 w-4" /> Zuletzt synchronisiert: {formatDateTime(provider.lastSync)}
                          </p>
                          <div className="mt-1 space-y-1 text-xs text-muted-foreground">
                            <p>
                              Datenabgleich: {provider.syncSettings.syncEntities.invoices ? 'Rechnungen' : '-'},
                              {' '}Kunden: {provider.syncSettings.syncEntities.customers ? 'Ja' : 'Nein'},
                              {' '}Zahlungen: {provider.syncSettings.syncEntities.payments ? 'Ja' : 'Nein'}
                            </p>
                            <p>
                              Token: {provider.credentials.tokenExpiry ? formatDateTime(provider.credentials.tokenExpiry) : 'Nicht gesetzt'}
                              {tokenExpired && (
                                <Badge variant="destructive" className="ml-2">Abgelaufen</Badge>
                              )}
                            </p>
                            {provider.lastError && (
                              <p className="text-destructive">Fehler: {provider.lastError}</p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {!provider.authenticated ? (
                            <Button
                              size="sm"
                              disabled={isConnecting}
                              onClick={() => handleAccountingConnect(provider)}
                              className="flex items-center gap-2"
                            >
                              <Plug className="h-4 w-4" />
                            ...
                            <Button
                              size="sm"
                              onClick={() => handleAccountingSync(provider.id)}
                              disabled={syncingProviderId === provider.id}
                              className="flex items-center gap-2"
                            >
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleAccountingRefreshToken(provider.id)}
                            className="flex items-center gap-2"
                          >
                            <RefreshCcw className="h-4 w-4" />
                            Token erneuern
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleAccountingReset(provider.id)}
                            className="flex items-center gap-2"
                          >
                            <Power className="h-4 w-4" />
                            Integration zuruecksetzen
                          </Button>
                        </div>
                        ...
                        <div className="space-y-2 rounded-md border p-3">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-sm font-medium">Webhook aktiv</p>
                              <p className="text-xs text-muted-foreground">Benachrichtigungen fuer Finanz-Ereignisse.</p>
                            </div>
                            <Switch
                              checked={provider.webhookActive}
                              onCheckedChange={(checked) => handleAccountingWebhookToggle(provider.id, checked)}
                            />
                          </div>
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleAccountingWebhookSave(provider.id)}
                                className="flex items-center gap-2"
                              >
                                <Link className="h-4 w-4" />
                                Speichern
